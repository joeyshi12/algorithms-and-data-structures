...
int GraphTools::findMinWeight(Graph& graph)
{
    ...
    Edge min = minWeightBFS(graph, vertices[0]);
...
int GraphTools::findShortestPath(Graph& graph, Vertex start, Vertex end)
{
    ...
    shortestPathBFS(graph, start, end, parent);
 ...
 */
Edge GraphTools::minWeightBFS(Graph& graph, Vertex start)
{
    queue<Vertex> q;
...
void GraphTools::shortestPathBFS(Graph& graph, Vertex start, Vertex end, unordered_map<Vertex, Vertex>& parent)
{
  queue<Vertex> q;
  graph.setVertexLabel(start, "VISITED");
  q.push(start);
...
{
  graph.setVertexLabel(v, "boop");
  vector<Vertex> adj = graph.getAdgacent(v);
  for (size_t i = 0; i < adj.size(); ++i) {
    if (graph.getVertexLabel(adj[i]) != "boop") {
      DFS(graph, adj[i]);
    }
  }
}
