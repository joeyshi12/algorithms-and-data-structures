...
private:
   ...
   */
   ...
   void prune(Node* & subRoot, int tol);

   bool canPrune(Node* & subRoot, int tol, RGBAPixel avg);

   int pruneSize(Node* & subRoot, int tol);

   int leaves(Node *&subRoot);

   void render(Node* subRoot, PNG &im);

   void clear(Node*& subRoot);

   Node* copy(const Node* subRoot) const;

   int findVSplitPoint(stats & s,pair<int,int> & ul, pair<int,int> & lr);

   int findHSplitPoint(stats & s,pair<int,int> & ul, pair<int,int> & lr);
